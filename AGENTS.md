...
- **`claude-review.yml` 自体を変更するPRでは、レビューは実行されない。** `anthropics/claude-code-action` はワークフローファイルがデフォルトブランチと同一であることを要求し、一致しなければスキップする。ワークフローを書き換えるPRが、その書き換えた権限のままマージ前に動くことを防ぐための機構である。**このときチェックは緑の `pass` になるが、レビューされたわけではない。**ログに `Skipping action due to workflow validation` が出る。ワークフローを変更するPRは、レビューなしでマージし、次のPRで効果を確認すること。
- **`claude-review.yml` は静的読解専用である。** テストを実行する権限を意図的に与えていない。このリポジトリは公開であり、`pytest` はリポジトリ内の任意のコードを実行するため。**テストが走っていないわけではない**（`tests.yml` が同じPRで実行する）。差分の外を読むための `Read` / `Glob` / `Grep` は許可してあるので、レビューが「diff範囲外なので確認していない」と述べていたら、それは権限ではなく指示の問題である。
