...
**7/31の原因の切り分け：** 08-05 07:15、ユーザーがTeslaアプリで車両を起こした際に `charge_state` を確認したところ、`scheduled_charging_mode = 'Off'`、`scheduled_charging_pending = False`、`off_peak_charging_enabled = False`、`preconditioning_enabled = False` だった。**車両側が時刻をトリガーに充電を開始する設定は入っていない**（`scheduled_departure_time` に7/23頃の古い値が残っていたが、モードが `Off` のため機能していない）。したがって7/31夜の充電再開は、アプリ・車内からの手動開始か、ケーブルの挿し直しによる自動開始のいずれかである。どちらかはログからは判別できない。いずれにせよ夜間監視が10分以内に検知して停止するため、実務上の影響は解消している。
