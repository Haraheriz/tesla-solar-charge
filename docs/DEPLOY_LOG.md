...
- 旧挙動（停止確認後は再問い合わせしない）を仕様として固定していたテストを削除し、再開ケースの回帰テスト2件を含む5件に差し替え（計31件）。新テストは修正前の実装では失敗することを確認済み
- `tesla_solar_charger.py` を転送し、`tesla-charger.service` を再起動

**結果：** `tesla-charger.service` `active (running)`、ローカルとラズパイのファイルがmd5一致（`b79b4665…`）。起動時のトークンリフレッシュ成功、車両（VIN捕捉）まで正常。夜間の新ログ（`車両は『offline』のため充電していないと判断し、そのまま休止します。`）を実機で確認。旧版の`夜間休止前の充電状態を確認します（1/6回目）...`は出力されなくなった。

**未確認：** 夜間に充電が再開する車両側の引き金（スケジュール充電・出発時刻設定・挿し直しのいずれか）は特定できていない。`charge_state.scheduled_charging_mode` を実機で確認するのは日中の稼働時に持ち越し。
