...
これを裏付ける読み取りとして、08-05 07:15（利用者がTeslaアプリで車両を起こした際）の `charge_state` は `scheduled_charging_mode = 'Off'`、`scheduled_charging_pending = False`、`off_peak_charging_enabled = False`、`preconditioning_enabled = False` だった。`scheduled_departure_time` に7/23頃の古い値が残っていたが、モードが `Off` のため機能していない。**ただしこれは08-05時点の設定であり、それ自体では07-31の状態を示さない**（設定は随時変更できるため）。判定の根拠は利用者への確認の方である。この区別を誤りかけた経緯は `docs/03_operation.md` 事象1.5 に注意として記載した。
