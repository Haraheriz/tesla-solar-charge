...
休止帯に入って最初のサイクルでのみ、充電が続いていないかを確認する。確認結果は必ず1行残るため、「夜間チェックが実際に走ったか」をログから追跡できる。

```text
[2026-05-17 18:00:00] [INFO] --- 定期チェック開始 ---
[2026-05-17 18:00:00] [INFO] 夜間休止モード中（現在時刻 18:00:00）
[2026-05-17 18:00:00] [INFO] 夜間休止前の充電状態を確認します（1/6回目）...
[2026-05-17 18:00:02] [INFO] 車両は『online』・充電状態『Stopped』のため、停止操作は不要と判断しました。
[2026-05-17 18:00:02] [INFO] 次の稼働チェックまで10分間スリープします...

```

確認結果は状況によって次のいずれかになる。上3つで確認完了となり、以降のサイクルでは車両へ問い合わせない。

| ログ | 意味 |
| --- | --- |
| `車両は『asleep』のため充電していないと判断し、そのまま休止します。` | 就寝中の車両は充電していないため起こさない |
| `車両は『online』・充電状態『Stopped』のため、停止操作は不要と判断しました。` | 起きているが充電していない |
| `夜間休止に入るため、進行中の充電を『一時停止』します。` → `充電の停止を確認しました。` | 充電中だったので停止させた |
| `夜間休止前の車両リスト取得に失敗しました（N/6回目）。次の巡回で再確認します。` | 通信不良。**確認完了とせず**10分後に再試行する |

2回目以降のサイクルは確認済みのため、以下の2行のみになる。
...
夜間休止に入る際、充電中であればシステムが `charge_stop` を送って止める。この停止は**成功が確認できるまで10分間隔で最大6回再試行**され、それでも確認できない場合は次のログを出す。

```text
[CRITICAL] 夜間休止前の充電停止を6回試みましたが確認できませんでした。系統からの充電が継続している可能性があります。手動で確認してください。
```

この行が出ていたら、Teslaアプリで実際の充電状態を確認し、必要なら手動で停止すること。原因は多くの場合プロキシ（4443番）または車両との通信不良なので、あわせて「事象1」「事象2」を確認する。
