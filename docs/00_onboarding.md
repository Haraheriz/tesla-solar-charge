...
テスラAPIには、2023年以前に使われていた古い非公式API（Owner API）の解説ブログや旧コードがネット上に大量に散乱している。本システムの拡張や修正を行う際は、以下の旧仕様を混入させないこと。

* **非公式オーナーAPI（`owner-api.teslamotors.com`）のURLは使用しない**
  → 現在は完全に廃止されており、アクセスしても一切動作しない。必ず公式の「Fleet API（Command Protocol）」および「ローカルHTTPプロキシ（TVCP）」ベースの構造を維持すること。
* **車両ディスプレイに「物理キーカード承認」のポップアップが表示された場合、それをスキップ・自動化するコードは書かない**
  → 鍵の登録自体はPhase 4の手順（ドメイン経由のVirtual Key Pairing）で完了できるが、車種・状況によっては車両側が追加で物理キーカードによる承認を要求することがある。これが表示された場合にプログラム側からバイパスする手段は存在しないため、必ずユーザーに物理キーカードでの承認を案内すること。